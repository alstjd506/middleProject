			...
			<table id="datatablesSimple">
				...
				<tbody>
					<c:forEach var="product" items="${productList }">
						<tr>
							<td><a class="productLink"
								href="adminProductInfo.do?prodNo=${product.prodNo }">${product.prodName }</a></td>
							...
							<c:choose>
								<c:when test="${cancle.orderStatus == 5 }">
									<td><a class="cancleLink"
										href="adminCancleAgree.do?ono=${cancle.orderNo }">취소요청</a></td>
								</c:when>
								<c:otherwise>
									<td>취소완료</td>
								</c:otherwise>
							...
							<c:choose>
								<c:when test="${returns.orderStatus == 7 }">
									<td><a class="returnLink"
										href="adminReturnAgree.do?rno=${returns.orderNo }">반품요청</a></td>
								</c:when>
								<c:otherwise>
									<td>반품완료</td>
								</c:otherwise>
